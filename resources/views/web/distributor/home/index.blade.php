    ...
    <div class="main-container min-h-[100vh] min-w-[100vw] bg-white dark:bg-gray-800 pt-12 ">
        ...
        <div id="Products" class="min-w-full bg-white dark:bg-gray-900 dark:text-white px-4 sm:px-6 lg:px-[5vw] pb-20">
            ...
            <div class="flex flex-col gap-3 px-3 md:px-5 w-full">
                <h1 class="text-4xl font-semibold ">Explore Our Products</h1>
                        ...
                        <a>
                            <div
                                ...
                                <div class="flex flex-col  gap-2 mt-3">
                                    ...
                                    <div x-data="{ cartList: [] }" x-init="cartList = JSON.parse(localStorage.getItem('cart') || '[]')" id="cart-btn-{{ $product->id }}"
                                        class="cart-btn flex  ">

                                        <button @guest
onclick="window.location.href='{{ route('login') }}'" @endguest
                                            @auth
onclick="addToCart({{ $product->id }})" @endauth
                                            x-show="!cartList.includes({{ $product->id }})" type="button"
                                            id="atc-{{ $product->id }}"
                                            class="min-w-[160px]  max-w-full text-white bg-gradient-to-r from-blue-500 via-blue-600 to-blue-700 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2 disabled:opacity-80
                                ">Add
                                            to cart</button>
...
onclick="window.location.href='{{ route('login') }}'" @endguest
                                            @auth
onclick="removeCart({{ $product->id }})" @endauth
                                            x-show="cartList.includes({{ $product->id }})" type="button"
                                            id="rmc-{{ $product->id }}"
                                            class="rmc hidden min-w-full  max-w-[180px] py-2.5 px-5 me-2 mb-2 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">Added</button>
                                    </div>
